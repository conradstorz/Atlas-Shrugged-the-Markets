...
- `scoring/engine.py` — the **concrete, active** ETF scorer, called by the CLI and web app. Four components: AI (a keyword heuristic on the fund's description), resilience (role keyword, eroded by real IT exposure), cost (real expense ratio), and diversification (measured breadth from an imported holdings file covering ≥90% of the fund). **A component whose input is missing is excluded rather than given a default**, and the remaining components share a fixed 80-point budget; a fund left with only the AI heuristic gets no overall score at all (`overall_score is None`). So `overall_score`, `resilience_score`, `cost_score` and `diversification_score` are all nullable, in the dataclass and in `etf_score`. Every score carries an explanation string naming each component's basis or why it was excluded. Still a prototype scorer (`SCORER_VERSION` in `scoring/model.py`), but it no longer substitutes invented values for absent evidence.
