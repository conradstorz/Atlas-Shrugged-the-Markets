...
- `db/database.py` + `db/schema.sql` — SQLite is the single source of truth. The schema is normalized: `asset(symbol, name, asset_type)` is the supertype registry for every symbol (`asset_type` is `'fund'` or `'company'`); `fund` and `company` are subtype tables keyed by the same symbol; `fund_holding` is keyed by `(fund_symbol, holding_symbol, source)` with FKs to `fund` and `asset`; `fund_score` mirrors the old `etf_score` columns and nullability; `theme` exists but is empty, awaiting v0.9. `connect()` applies `schema.sql` (idempotent `CREATE TABLE IF NOT EXISTS`) and then runs `db/migrations.py`'s `PRAGMA user_version` migration runner, which normalizes a legacy database's old `etf`/`etf_holding`/`etf_score` tables into the new ones in place. Loaders parse the seed CSV's pipe-delimited top-ten holdings format (`|NVDA||AAPL|`) into the `fund_holding` table.
- `scoring/engine.py` — the **concrete, active** ETF scorer, called by the CLI and web app. Four components: AI (a keyword heuristic on the fund's description), resilience (role keyword, eroded by real IT exposure), cost (real expense ratio), and diversification (measured breadth from an imported holdings file covering ≥90% of the fund). **A component whose input is missing is excluded rather than given a default**, and the remaining components share a fixed 80-point budget; a fund left with only the AI heuristic gets no overall score at all (`overall_score is None`). So `overall_score`, `resilience_score`, `cost_score` and `diversification_score` are all nullable, in the dataclass and in `fund_score`. Every score carries an explanation string naming each component's basis or why it was excluded. Still a prototype scorer (`SCORER_VERSION` in `scoring/model.py`), but it no longer substitutes invented values for absent evidence.
