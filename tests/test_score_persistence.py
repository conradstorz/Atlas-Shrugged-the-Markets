...
def test_read_scores_returns_persisted_rows_without_recomputing(tmp_path: Path) -> None:
    ...
    conn.execute("UPDATE fund_score SET overall_score = 7 WHERE symbol = 'AAA'")
    conn.commit()
...
def test_read_scores_is_sorted_by_overall_score_desc(tmp_path: Path) -> None:
    ...
    _seed_two_etfs(conn)
    score_all(conn)
    conn.execute("UPDATE fund_score SET overall_score = 90 WHERE symbol = 'BBB'")
    conn.execute("UPDATE fund_score SET overall_score = 10 WHERE symbol = 'AAA'")
    conn.commit()
...
def test_read_scores_does_not_write_to_the_database(tmp_path: Path) -> None:
    ...
    before = conn.execute("SELECT symbol, overall_score FROM fund_score ORDER BY symbol").fetchall()
    read_scores(conn)
    after = conn.execute("SELECT symbol, overall_score FROM fund_score ORDER BY symbol").fetchall()
...
def test_persisted_scores_are_real_numbers_before_any_test_overwrites_them(
    ...
    stored = conn.execute("SELECT overall_score FROM fund_score ORDER BY symbol").fetchall()
